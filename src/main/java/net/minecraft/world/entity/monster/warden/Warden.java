...
public class Warden extends Monster implements VibrationListener.VibrationListenerConfig {
   ...
   public void readAdditionalSaveData(CompoundTag p_219415_) {
      ...
      if (p_219415_.contains("listener", 10)) {
         VibrationListener.codec(this).parse(new Dynamic<>(NbtOps.INSTANCE, p_219415_.getCompound("listener"))).resultOrPartial(LOGGER::error).ifPresent((p_269962_) -> {
            this.dynamicGameEventListener.updateListener(p_269962_, this.level);
         });
      }
   ...
   public boolean shouldListen(ServerLevel p_219370_, GameEventListener p_219371_, BlockPos p_219372_, GameEvent p_219373_, GameEvent.Context p_219374_) {
      if (!this.isNoAi() && !this.isDeadOrDying() && !this.getBrain().hasMemoryValue(MemoryModuleType.VIBRATION_COOLDOWN) && !this.isDiggingOrEmerging() && p_219370_.getWorldBorder().isWithinBounds(p_219372_)) {
