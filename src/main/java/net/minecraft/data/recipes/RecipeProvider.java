...
public abstract class RecipeProvider implements DataProvider {
   ...
   private static void oreCooking(Consumer<FinishedRecipe> p_250791_, RecipeSerializer<? extends AbstractCookingRecipe> p_251817_, List<ItemLike> p_249619_, RecipeCategory p_251154_, ItemLike p_250066_, float p_251871_, int p_251316_, String p_251450_, String p_249236_) {
   ...
   protected static void legacyNetheriteSmithing(Consumer<FinishedRecipe> p_267010_, Item p_267129_, RecipeCategory p_266966_, Item p_267096_) {
      LegacyUpgradeRecipeBuilder.smithing(Ingredient.of(p_267129_), Ingredient.of(Items.NETHERITE_INGOT), p_266966_, p_267096_).unlocks("has_netherite_ingot", has(Items.NETHERITE_INGOT)).save(p_267010_, getItemName(p_267096_) + "_smithing");
   }
   ...
   protected static void trimSmithing(Consumer<FinishedRecipe> p_267135_, Item p_266861_) {
      SmithingTrimRecipeBuilder.smithingTrim(Ingredient.of(p_266861_), Ingredient.of(ItemTags.TRIMMABLE_ARMOR), Ingredient.of(ItemTags.TRIM_MATERIALS), RecipeCategory.MISC).unlocks("has_smithing_trim_template", has(p_266861_)).save(p_267135_, getItemName(p_266861_) + "_smithing_trim");
   }
   ...
   private static void nineBlockStorageRecipes(Consumer<FinishedRecipe> p_250423_, RecipeCategory p_250083_, ItemLike p_250042_, RecipeCategory p_248977_, ItemLike p_251911_, String p_250475_, @Nullable String p_248641_, String p_252237_, @Nullable String p_250414_) {
   ...
   private static void simpleCookingRecipe(Consumer<FinishedRecipe> p_249398_, String p_249709_, RecipeSerializer<? extends AbstractCookingRecipe> p_251876_, int p_249258_, ItemLike p_250669_, ItemLike p_250224_, float p_252138_) {
