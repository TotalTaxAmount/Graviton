...
public class Font {
   ...
   private int drawInternal(String p_254064_, float p_254270_, float p_254152_, int p_254242_, Matrix4f p_254345_, boolean p_254516_, boolean p_254277_) {
      if (p_254064_ == null) {
         ...
         int i = this.drawInBatch(p_254064_, p_254270_, p_254152_, p_254242_, p_254516_, p_254345_, multibuffersource$buffersource, Font.DisplayMode.NORMAL, 0, 15728880, p_254277_);
   ...
   private int drawInternal(FormattedCharSequence p_253879_, float p_254094_, float p_254476_, int p_254424_, Matrix4f p_253964_, boolean p_254201_) {
      MultiBufferSource.BufferSource multibuffersource$buffersource = MultiBufferSource.immediate(Tesselator.getInstance().getBuilder());
      int i = this.drawInBatch(p_253879_, p_254094_, p_254476_, p_254424_, p_254201_, p_253964_, multibuffersource$buffersource, Font.DisplayMode.NORMAL, 0, 15728880);
      multibuffersource$buffersource.endBatch();
      return i;
   }
   ...
   public void drawWordWrap(PoseStack p_275601_, FormattedText p_275493_, int p_275609_, int p_275274_, int p_275484_, int p_275332_) {
      Matrix4f matrix4f = p_275601_.last().pose();

      for(FormattedCharSequence formattedcharsequence : this.split(p_275493_, p_275484_)) {
         this.drawInternal(formattedcharsequence, (float)p_275609_, (float)p_275274_, p_275332_, matrix4f, false);
         p_275274_ += 9;
      }
