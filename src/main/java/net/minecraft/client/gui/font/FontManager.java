...
public class FontManager implements AutoCloseable {
   ...
   private final PreparableReloadListener reloadListener = new SimplePreparableReloadListener<Map<ResourceLocation, List<GlyphProvider>>>() {
      protected Map<ResourceLocation, List<GlyphProvider>> prepare(ResourceManager p_95024_, ProfilerFiller p_95025_) {
         ...
         for(Map.Entry<ResourceLocation, List<Resource>> entry : FontManager.FONT_DEFINITIONS.listMatchingResourceStacks(p_95024_).entrySet()) {
            ResourceLocation resourcelocation = entry.getKey();
            ResourceLocation resourcelocation1 = FontManager.FONT_DEFINITIONS.fileToId(resourcelocation);
               ...
               try (Reader reader = resource.openAsReader()) {
                  try {
                     ...
                     for(int i = jsonarray.size() - 1; i >= 0; --i) {
                        ...
                        GlyphProviderBuilderType glyphproviderbuildertype = GlyphProviderBuilderType.byName(s);
