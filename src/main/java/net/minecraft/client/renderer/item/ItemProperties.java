...
public class ItemProperties {
   ...
   static {
      ...
      ClampedItemPropertyFunction clampeditempropertyfunction = (p_269748_, p_269749_, p_269750_, p_269751_) -> {
         if (!p_269748_.is(ItemTags.TRIMMABLE_ARMOR)) {
            return Float.NEGATIVE_INFINITY;
         } else if (p_269749_ == null) {
            return 0.0F;
         } else {
            return !p_269749_.enabledFeatures().contains(FeatureFlags.UPDATE_1_20) ? Float.NEGATIVE_INFINITY : ArmorTrim.getTrim(p_269749_.registryAccess(), p_269748_).map(ArmorTrim::material).map(Holder::value).map(TrimMaterial::itemModelIndex).orElse(0.0F);
         }
      };
